...
public class TorpedoLauncher : MonoBehaviour // Unityのゲームオブジェクトにアタッチするための基本クラス
{
    // ==========================================================
    // 外部連携用の変数（Inspectorで設定）
    // ==========================================================

    // 💡 右コントローラーのトリガーアクションをInspectorで設定
    public InputActionReference fireActionRight;

    // 💡 新規追加: 左コントローラーのトリガーアクションをInspectorで設定
    ...
    void Start() // ゲーム開始時に一度だけ実行される
    {
        ...
        {
            // 右コントローラーのPerformedイベントにメソッドを購読
        ...
        {
            // 左コントローラーのPerformedイベントにメソッドを購読
    ...
    // 右または左のアクションに接続されるメインのメソッド
    // どちらのコントローラーがトリガーしても、このメソッドが実行される
    public void OnFireTorpedo(InputAction.CallbackContext context)
    {
        if (context.performed)
        {
            ...
            if (torpedoPrefab != null && launchPoint != null)
            {
                // 1. 弾の生成 (Instantiate)
                ...
                if (rb != null)
                {
                    // LaunchPointの正面方向（Transform.forward）に力を加える
...
//            {
//                // 1. 弾の生成 (Instantiate)
//                //    設定された TorpedoPrefab を LaunchPoint の位置と回転でシーンに複製する
//                GameObject torpedo = Instantiate(torpedoPrefab, launchPoint.position, launchPoint.rotation);
...
//                if (rb != null)
//                {
//                    // LaunchPointの正面方向（Transform.forward）に、
//                    // LaunchForceで設定した強さの力を瞬間的に加える (ForceMode.Impulse)
//                    rb.AddForce(launchPoint.forward * launchForce, ForceMode.Impulse);
//                }
//            }
//        }
